...
func mustGetInboxHandler() (InboxController, func()) {
...
func shouldExistInbox(t *testing.T, ih InboxController, i model.Inbox) model.Inbox {
	w := httptest.NewRecorder()
	ginCtx, _ := gin.CreateTestContext(w)
...
func getInbox(t *testing.T, ih InboxController, id uuid.UUID) model.Inbox {
	w := httptest.NewRecorder()
	ginCtx, _ := gin.CreateTestContext(w)
