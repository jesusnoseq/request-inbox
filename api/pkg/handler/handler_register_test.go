...
func mustGetInboxHandler() (InboxService, func()) {
...
func shouldExistInbox(t *testing.T, ih InboxService, i model.Inbox) model.Inbox {
	w := httptest.NewRecorder()
	ginCtx, _ := gin.CreateTestContext(w)
...
func getInbox(t *testing.T, ih InboxService, id uuid.UUID) model.Inbox {
	w := httptest.NewRecorder()
	ginCtx, _ := gin.CreateTestContext(w)
